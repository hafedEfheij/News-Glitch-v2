...
import { AlertTriangle, CheckCircle, Loader2, ScrollText, Search, HelpCircle, XCircle, ExternalLink } from 'lucide-react';
...
export function NewsArticleProcessor() {
  ...
  async function onAnalyzeSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      const articleContent = await extractArticleContent(values.articleUrl);

      if (!articleContent) {
        setAnalysisError('تعذر استخراج محتوى المقال. يرجى التحقق من الرابط أو المحاولة مرة أخرى.'); // Failed to extract article content. Please check the URL or try again.
      ...
      const [summaryResponse, fakeNewsResponse] = await Promise.allSettled([
        summarizeArticle({
          ...
          articleContent: articleContent // Pass the extracted content to the summarize function
        }),
        detectFakeNews({
          articleUrl: values.articleUrl,
          articleContent: articleContent // Pass the extracted content to the fake news detection
        }),
      ]);
      ...
      if (summaryResponse.status === 'rejected') {
        ...
        errorMessages.push('فشل في تلخيص المقال');
      }

      if (fakeNewsResponse.status === 'rejected') {
        console.error("فشل كشف الأخبار الكاذبة:", fakeNewsResponse.reason);
        errorMessages.push('فشل في تحليل مصداقية المقال');
      }

      if (errorMessages.length > 0) {
        setAnalysisError(`حدث خطأ: ${errorMessages.join(', ')}. الرجاء المحاولة مرة أخرى.`);
      }
  ...
  return (
    <Card className="w-full shadow-lg border border-primary/20 rounded-xl overflow-hidden bg-card">
      {/* Article Analysis Section */}
      <CardHeader className="bg-primary/5 p-4 sm:p-6 border-b border-primary/10">
        ...
        <CardDescription className="text-sm sm:text-base">أدخل رابط المقال الإخباري أدناه للحصول على ملخص وتحليل للمعلومات المضللة المحتملة.</CardDescription> {/* Enter the URL of a news article below for a summary and analysis of potential misinformation. */}
      </CardHeader>
                ...
                <FormItem>
                  ...
                  <div className="flex flex-col sm:flex-row gap-2">
                    ...
                    <Button
                      ...
                      className="w-full sm:w-auto min-w-[100px] sm:min-w-[120px] text-sm sm:text-base"
                    >
                      {isAnalysisLoading ? (
                        <>
            ...
            {analysisResults.summaryResult ? (
              <div className="space-y-3">
                ...
                <p className="text-foreground/90 leading-relaxed bg-secondary/30 p-3 sm:p-4 rounded-md border text-sm sm:text-base">
                  {cleanTextForDisplay(analysisResults.summaryResult.summary)}
                </p>
              </div>
            ) : (
                     ...
                     analysisResults.fakeNewsResult.isFakeNews
                      ...
                      <div className="flex-grow">
                       ...
                       <p className={`text-xs sm:text-sm ${analysisResults.fakeNewsResult.isFakeNews ? 'text-destructive/90' : 'text-[hsl(var(--success))] opacity-90'}`}> {/* Use success color variable */}
                         {cleanTextForDisplay(analysisResults.fakeNewsResult.reason)}
                       </p>
                     </div>
                     <TooltipProvider>
                       <Tooltip>
      ...
      {currentUrl && !isAnalysisLoading && (
        <CardFooter className="bg-card-foreground/5 p-3 sm:p-4 border-t">
            <div className="flex items-center gap-2 text-xs sm:text-sm text-muted-foreground truncate">
              ...
              <span>الرابط الذي تم تحليله:</span>
              <ClientLink href={currentUrl} target="_blank" rel="noopener noreferrer" className="underline hover:text-primary truncate">{currentUrl}</ClientLink> {/* Analyzed URL: */}
            </div>
        </CardFooter>
      )}
        ...
        <CardHeader className="p-4 sm:p-6 bg-primary/5 border-y border-primary/10">
            <div className="flex items-center gap-2 mb-2">
                ...
                <div className="pt-4 space-y-4">
                    ...
                    >
                        ...
                        <AlertDescription className={`mt-1 ${verificationResult.verdict === "مرجح أنه صحيح" ? 'text-green-700/90' : verificationResult.verdict === "غير مؤكد" ? 'text-yellow-700/90' : ''}`}>
                            {cleanTextForDisplay(verificationResult.explanation)}
                        </AlertDescription>
                    </Alert>
                                ...
                                {verificationResult.supportingEvidence.map((evidence, index) => (
                                    <li key={index}>
                                        {evidence.startsWith('http') ? (
                                            ...
                                            <span className="break-words">{cleanTextForDisplay(evidence)}</span>
                                        )}
                                    </li>
                                ))}
                            </ul>
             ...
             {claimToVerify && !isVerificationLoading && (
                 <CardFooter className="bg-card-foreground/5 p-3 sm:p-4 border-t mt-4">
                     <div className="flex items-start gap-2 text-xs sm:text-sm text-muted-foreground">
                         ...
                         <div>
                             <span>النص الذي تم التحقق منه:</span>
                             <p className="mt-1 italic text-foreground/80 font-medium">"{cleanTextForDisplay(claimToVerify)}"</p>
                         </div>
                     </div>
                 </CardFooter>
             )}
